(function(){
  ...
  let plansDB = {
    '40 Mbps': {
      name: 'Starter Link',
      desc: 'Social Media & Browsing',
      benefits: 'Unlimited Data + 24/7 Support',
      base: { locations: ['Tiruppur', 'Pollachi'], monthly: 399, quarterly: 1197, halfyearly: 2394, yearly: 4788 },
      ott: { locations: ['Pollachi'], monthly: 499, quarterly: 1497, halfyearly: 2994, yearly: 5988 }
    },
    '100 Mbps': {
      name: 'Value Link',
      desc: 'Families & Students',
      benefits: 'Unlimited Data + HD Streaming',
      base: { locations: ['Tiruppur', 'Pollachi'], monthly: 599, quarterly: 1797, halfyearly: 3594, yearly: 7188 },
      ott: { locations: ['Tiruppur', 'Pollachi'], monthly: 699, quarterly: 2097, halfyearly: 4194, yearly: 8388 }
    },
    '200 Mbps': {
      name: 'Turbo Link',
      desc: 'Pro Gamers & Streamers',
      benefits: 'Unlimited Data + 4K Streaming',
      base: { locations: ['Tiruppur', 'Pollachi'], monthly: 899, quarterly: 2697, halfyearly: 5394, yearly: 10788 },
      ott: { locations: ['Tiruppur'], monthly: 999, quarterly: 2997, halfyearly: 5994, yearly: 11988 }
    },
    '500 Mbps': {
      name: 'Giga Link',
      desc: 'Power Users & Large Homes',
      benefits: 'Priority Routing + Ultra-Low Latency',
      base: { locations: ['Tiruppur'], monthly: 1399, quarterly: 4197, halfyearly: 8394, yearly: 16788 },
      ott: { locations: ['Tiruppur'], monthly: 1499, quarterly: 4497, halfyearly: 8994, yearly: 17988 }
    }
  };
    ...
    const planTypes = [
      {
        ...
        icon: 'fa-tower-cell',
        badge: speed === '100 Mbps' ? '🔥 POPULAR' : '',
        desc: data.desc,
        nameSuffix: 'Essential',
        ottEmpty: true
      },
      {
        key: 'ott',
        ...
        icon: 'fa-film',
        badge: speed === '200 Mbps' ? '🔥 BEST VALUE' : '',
        desc: `${data.desc} + Entertainment`,
        nameSuffix: 'OTT Bundle',
        ottEmpty: false
      }
    ];

    const filteredPlanTypes = planTypes.filter(type => {
      const planConfig = data[type.key];
      return planConfig && planConfig.locations && planConfig.locations.includes(activeLocation);
    });

    filteredPlanTypes.forEach(type => {
      const card = document.createElement('div');
      card.className = 'plan-card';
      if (type.badge) {
        card.innerHTML = `<div class="popular-badge">${type.badge}</div>`;
      }

      // Title & Header
      const nameText = type.key === 'ott' ? `${data.name} + OTT` : data.name;
      let cardHTML = `
        <div class="plan-header">
          <div class="plan-type"><i class="fas ${type.icon}"></i> ${type.title}</div>
          <div class="plan-name">${nameText}</div>
          <p class="plan-desc">${type.desc}</p>
        </div>
        <div class="plan-body">
          <div class="plan-benefits-text">
            <i class="fas fa-check-circle" style="color:var(--primary); margin-right:6px;"></i> ${data.benefits}
          </div>
      ...
      const monthlyBasePrice = data[type.key].monthly;
      const discounts = data.discounts || { monthly: 0, quarterly: 5, halfyearly: 10, yearly: 16.67 };

      durations.forEach((dur, index) => {
        const standardTotal = monthlyBasePrice * dur.months;
        const discountPercent = discounts[dur.key] || 0;
        const price = Math.round(standardTotal * (1 - discountPercent / 100));
        const defaultChecked = index === 0; // Default to Monthly
        
        let offerBadgeHTML = '';
        const roundedDiscountPercent = Math.round(discountPercent);
        if (roundedDiscountPercent > 0) {
          const extraText = dur.months === 12 ? ' (2 Mos Free!)' : '';
          offerBadgeHTML = `<span class="offer-percent-badge">Save ${roundedDiscountPercent}%${extraText}</span>`;
        }

        const formattedPricePerMonth = Math.round(price / dur.months).toLocaleString('en-IN');
        const originalPriceHTML = roundedDiscountPercent > 0 ? `<span class="duration-original-price">₹${standardTotal.toLocaleString('en-IN')}</span>` : '';

        cardHTML += `
          <div class="duration-option ${defaultChecked ? 'selected' : ''}" onclick="selectDurationOption(this, '${speed}', '${type.key}', '${dur.key}')">
            <input type="radio" class="duration-radio" name="duration-${speed}-${type.key}" value="${dur.key}" ${defaultChecked ? 'checked' : ''} style="display:none;" />
            <div class="duration-info">
              <div class="duration-details">
                ...
                <span class="duration-price-per-mo">₹${formattedPricePerMonth}/mo</span>
              </div>
              <div class="duration-total-price-wrap">
                <span class="duration-total-price">₹${price.toLocaleString('en-IN')}</span>
                ${originalPriceHTML}
              </div>
            </div>
          </div>`;
      ...
      // Render OTT benefits preview
      if (type.ottEmpty) {
        cardHTML += `
          <div class="ott-preview-box empty">
            <div class="ott-badge-muted"><i class="fas fa-ban"></i> No OTT Apps Included</div>
            <div class="ott-placeholder-text">Standard high-speed data connectivity without streaming subscriptions.</div>
          </div>`;
      } else {
        cardHTML += `
      ...
      cardHTML += `
        ...
        <div class="plan-action">
          <button class="btn-select" onclick="handleSelectPlan('${speed}', '${type.key}')">Select Plan →</button>
        </div>`;
  ...
  window.handleSelectPlan = function(speed, typeKey) {
    ...
    const planLabel = typeKey === 'ott' ? `${data.name} OTT Bundle` : `${data.name} Essential`;
    const durationMonths = { monthly: 1, quarterly: 3, halfyearly: 6, yearly: 12 };
    const months = durationMonths[durationKey];
    const monthlyBasePrice = data[typeKey].monthly;
    const standardTotal = monthlyBasePrice * months;
    const discounts = data.discounts || { monthly: 0, quarterly: 5, halfyearly: 10, yearly: 16.67 };
    const discountPercent = discounts[durationKey] || 0;
    const price = Math.round(standardTotal * (1 - discountPercent / 100));

    const fullPlanName = `${planLabel} (${speed}) - ${durationLabels[durationKey]} (₹${price.toLocaleString('en-IN')})`;
  ...
  function updateSpeedTabsVisibility() {
    ...
    tabs.forEach(tab => {
      ...
      // A speed is available if either its base or ott plan has activeLocation in locations
      const isBaseAvailable = planData && planData.base && planData.base.locations && planData.base.locations.includes(activeLocation);
      const isOttAvailable = planData && planData.ott && planData.ott.locations && planData.ott.locations.includes(activeLocation);
      const isAvailable = isBaseAvailable || isOttAvailable;
